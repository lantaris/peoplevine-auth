...
func handleBind(w ldap.ResponseWriter, m *ldap.Message) {
	...
	if r.AuthenticationChoice() == "simple" {
		if string(r.Name()) == "test" && string(r.AuthenticationSimple()) == "test123" {
			w.Write(res)
			return
		}
...
func handleSearch(w ldap.ResponseWriter, m *ldap.Message) {
	...
	e := ldap.NewSearchResultEntry("cn=TEST, " + string(r.BaseObject()))
	e.AddAttribute("mail", "test@gmail.com")
	e.AddAttribute("company", "KOKO")
	e.AddAttribute("department", "")
	e.AddAttribute("l", "")
	e.AddAttribute("mobile", "")
	e.AddAttribute("telephoneNumber", "")
	e.AddAttribute("cn", "")
	w.Write(e)
